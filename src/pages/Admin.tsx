...
const Admin = () => {
  ...
  const [showMotoboyLogin, setShowMotoboyLogin] = useState(false);
  ...
  const handleMotoboyClick = useCallback(() => {
    setShowMotoboyLogin(true);
  }, []);
      ...
      <div className="relative z-10 w-full lg:max-w-6xl mx-auto min-h-screen bg-black/70 p-4 content-overlay">
        {isAuthenticated ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-screen">
            ...
            {showMotoboyLogin ? (
              <MotoboyLogin onLogin={handleMotoboyLogin} onBackToAdmin={handleBackToAdmin} />
            ) : (
              <AdminLogin onLogin={handleLogin} onMotoboyClick={handleMotoboyClick} />
            )}
          </div>
        )}
      </div>
